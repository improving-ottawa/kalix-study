...
class MemberByMemberIdsQueryView(context: ViewContext)
    extends AbstractMemberByMemberIdsQueryView {
  ...
  override def processRegisterMember(
    ...
    if (state != emptyState) {
      ...
      log.info(
        s"MemberByMemberIdsQueryView in processRegisterMember - state already existed"
      )
      ...
      log.info(
        s"MemberByMemberIdsQueryView in processRegisterMember - memberRegistered ${memberRegistered}"
      )
  ...
  override def processUpdateMemberStatus(
    ...
    log.info(
      s"MemberByMemberIdsQueryView in processUpdateMemberStatus - memberStatusUpdated ${memberStatusUpdated}"
    )
  ...
  override def processUpdateMemberInfo(
    ...
    log.info(
      s"MemberByMemberIdsQueryView in processUpdateMemberInfo - memberInfoUpdated ${memberInfoUpdated}"
    )
