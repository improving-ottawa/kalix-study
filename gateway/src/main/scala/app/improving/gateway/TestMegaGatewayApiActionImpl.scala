...
class TestMegaGatewayApiActionImpl(creationContext: ActionCreationContext)
  ...
  val tenantService: TenantService =
    creationContext.getGrpcClient(
      ...
      config.getString("app.improving.gateway.mega.grpc-client-name")
    )

  val organizationService: OrganizationService = creationContext.getGrpcClient(
    classOf[OrganizationService],
    config.getString(
      "app.improving.gateway.mega.grpc-client-name"
    )
  )
  ...
  override def handleStartScenario(
    ...
    memberService
      ...
      .value match {
      case Some(Success(msg)) => log.info(s"registerMember ${msg}")
      case Some(Failure(exception)) =>
        log.info(s"registerMember ${exception.getMessage}")
      case None => log.info("None from registermember")
    }
